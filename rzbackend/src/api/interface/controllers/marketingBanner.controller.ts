...
import { storeMarketingBannerModel,getMarketingBannerDetail } from "../../domain/models/marketingBanner.model";
...
export const updateMarketingBanner = async (req: Request, res: Response) => {
    try {
        ...
        const categories = await storeMarketingBannerModel(req.body, (error:any, result:any) => {
            if (error) {
                ErrorResponse(res,error.message);
            }
            storeUserActionActivity(req.user.userId, "marketingBanner", "Update", `Update marketing Banner Details successfully`);
            return successResponse(res, "Update marketing Banner successfully", result);
        });
...
export const marketingBannerDetails = async (req: Request, res: Response) => {
    try {
        getMarketingBannerDetail(req.user, (error: any, result: any) => {
            if (error) {
                console.error("Error:", error);
                return ErrorResponse(res, error.message);
            }

            return successResponse(res, "Marketing Banner details", result); 
        });
