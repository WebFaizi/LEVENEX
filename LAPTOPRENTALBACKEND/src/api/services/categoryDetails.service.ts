...
function replaceAnchorHrefWithLocation(metaDescription: string | undefined, currentUrl: string): string {
  if (!metaDescription || !metaDescription.includes('<a')) return metaDescription || '';
...
export const getCategoryDetails = async (
    ...
    try {
        ...
        const slugPart = slugify(`${category.slug} ${replacements_location.location}`, {
        lower: true,
        strict: true, // remove special characters
        });
        const url = process.env.BASE_URL_TWO+ '/'+slugPart+'/'+category.unique_id;
        category.description = await replaceAnchorHrefWithLocation(category.description, url);
        await Promise.all([
            category.description && (category.description = await replacePlaceholders(category.description, replacements_location)),
            category.subdomain_description && (category.subdomain_description = await replacePlaceholders(category.subdomain_description, replacements_location)),
            category.page_top_descritpion && (category.page_top_descritpion = await replacePlaceholders(category.page_top_descritpion, replacements_location)),
            category.page_top_keyword && (category.page_top_keyword = await replacePlaceholders(category.page_top_keyword, replacements_location)),
        ]);
